...
```{r download-actual, warning = FALSE}

resource_name <- "relative_abundance"
filename <- paste("ps_list", paste0(dim(studies)[1], "samples"), resource_name, "Rda", sep=".")

if (download_data == TRUE) {
    ps_list <- list()
    tmp_name_list <- studies$study_name
    for (name in tmp_name_list) {
      tse <- curatedMetagenomicData(paste(name, resource_name, sep="."), dryrun = FALSE, counts = FALSE, rownames = "short") %>%
        mergeData()
      ps <- makePhyloseqFromTreeSummarizedExperiment(tse, abund_values = "relative_abundance")
      ps_list[[name]] <- ps
    }
    save(ps_list, file = filename)
} else {
    load(filename)
}

name_list <- names(ps_list)
...
# Comment the following line if this is a real analysis. We use 4 datasets only in an example run.
#name_list <- c("FengQ_2015", "VogtmannE_2016", "YuJ_2015", "ZellerG_2014")
filename = paste("tmp_results", paste0(length(name_list), "samples"), paste0(min_prev_percent, "prev"), "relative_abundance", "Rda", sep=".")
if (run_analysis == TRUE) {
    ...
    save(tmp_results, file = filename)
} else {
    load(filename)
}
...
The full list is quite long, with `r dim(exclusion[["WirbelJ_2018"]])[1]` exclusions. *s_i* and *s_j* are the two microbes who exhibit a co-exclusion pattern. They occur at $p_i$ and $p_j$ probabilities, respectively, in this dataset. These probabilities are given in columns *p_i* and *p_j*. Co-exclusion is inferred because even though their expected joint probability $p_i \times p_j$ is non-zero (in column *p_i_TIMES_p_j*), they never co-occur as seen by the real joint probability $p_{ij}$ (in column *p_i_AND_j*). This breaks the independence assumption towards the direction that they potentially exclude each other. The column *log2DEP* is derived from the independence measure defined as: $$\log_2\left(\frac{p_i \times p_j}{p_{ij}}\right)$$
...
```{r eur-studies, echo=TRUE, eval=TRUE}
...
```{r eur-studies-results, echo=TRUE, eval=TRUE}
...
filename = paste("co_exclusions", paste0(length(name_list), "samples"), paste0(min_prev_percent, "prev"), "relative_abundance", "xlsx", sep=".")
...
```{r visualize-example1, fig.width = 10, fig.height = 9, fig.cap = "Co-exclusion pattern of two taxa observed in multiple datasets",eval = TRUE, echo = TRUE}
library(ggplot2)

t1="Dialister invisus"
t2="Phascolarctobacterium succinatutens"

plot_interaction_phyloseq(ps_list_filtered, t1, t2)
```
...
```{r visualize-example2, fig.width = 10, fig.height = 9, fig.cap = "Co-exclusion pattern of two taxa observed in multiple datasets",eval = TRUE, echo = TRUE}
library(ggplot2)
...
prev_species <- c()
for (study in name_list) {
  prev_species <- otu_table(ps_list_filtered[[study]]) %>% 
    as.data.frame %>% 
    rowSums %>% 
