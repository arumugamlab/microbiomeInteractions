...
filename = paste("taxa_pairwise", 
                 paste0(length(name_list), "samples"), 
...
if (run_analysis == TRUE) {
    ...
    save(taxa_pairwise, file = filename)
} else {
    load(filename)
}
...
```{r taxa-peek-df}
taxa_pairwise[["WirbelJ_2018"]] %>%
  mutate(log2DEP = as.character(round(log2DEP, 4))) %>%
  head %>%
  datatable(
    caption = htmltools::tags$caption(
      style = 'caption-side: bottom; text-align: center;',
      "Top 6 co-exclusions from the", htmltools::code("WirbelJ_2018"), " dataset"
      )
    ) %>%
...
The full list is quite long, with `r dim(exclusion[["WirbelJ_2018"]])[1]` exclusions. $s_i$ and $s_j$ are the two microbes who exhibit a co-exclusion pattern (columns `s_i` and `s_j`). They occur at $p_i$ and $p_j$ probabilities, respectively, in this dataset. These probabilities are given in columns `p_i` and `p_j`. If their individual occurrences are independent from each other, the expected probability that they both occur in a sample is then given by the joint probability $p_i \times p_j$. We have also counted how often they actually occur together, $p_{ij}$, which we can compare against $p_i \times p_j$ to see whether they are indeed independent from each other.

```{r visualize-dependence-1, fig.width = 5, fig.asp = 1, fig.cap = "Comparing expected and actual co-occurrence probabilities",}

ggplot(taxa_pairwise$WirbelJ_2018, aes(x=p_i_AND_j, y=p_i_TIMES_p_j, )) + 
  geom_point(alpha=0.05) + 
  geom_function(fun=function(x){2*x}) +
  geom_function(fun=function(x){x/2}) +
  geom_smooth(formula = y~x, color="red", se=TRUE, level=0.9999) +
  ylim(0,1)
...
```{r taxa-top-exc}
top_exc <- get_top_exclusions(name_list, taxa_pairwise, max_log2DEP = 10)
```
...
top_exc[["WirbelJ_2018"]] %>%
  ...
  datatable(
    caption = htmltools::tags$caption(
      style = 'caption-side: bottom; text-align: center;',
      "Top 6 co-exclusions from the", htmltools::code("WirbelJ_2018"), " dataset, now rescored"
    )
  ) %>%
...
probs <- data.frame(study=name_list, n_samples = studies$n, exc_prob=exc)
hist(probs$exc_prob, xlab = "probability", main = "Histogram of exclusion probabilities")
```
...
plot(probs$n_samples, probs$exc_prob)
cor.test(probs$n_samples, probs$exc_prob, method="spearman")
```
...
mean_exc <- mean(as.numeric(probs$exc_prob))
```

The estimate for $\pi$ is `r mean_exc`. For your own datasets, if you cannot estimate it, by default `get_overlapping_exclusions()` uses $\pi=0.3$, which we estimated from all the studies listed above.
...
library(ggplot2)
hist(dependency$WirbelJ_2018$p_i_AND_j)
ggplot(dependency$WirbelJ_2018, aes(x=p_i_OR_j, y=p_i_AND_j, )) + 
  geom_point(alpha=0.05) + 
  #geom_density_2d() +
...
dep <- c()
for (study in name_list) {
  n_d     <- sum(exclusion[[study]]$log2DEP > 1 )
  n_total <- choose(ntaxa(taxa_ps_list_filtered[[study]]), 2)
...
```{r taxa-depend, eval = FALSE, echo = FALSE, message = FALSE}
top_dep <- get_top_dependencies(name_list, exclusion, min_log2DEP = 1)
...
```{r taxa-depend-table, eval = FALSE, echo = FALSE, message = FALSE}
global_dep_overlap %>%
...
inner_join(global_overlap, global_dep_overlap, by=c("species_i", "species_j")) %>% dim
